 ...
 *
 *  @param View Distance
 *  @desc N?mero de tiles ? frente que o jogador v? por padr?o.
 *  @default 3
 *
 *  @param Light Color
 *  @desc C?digo hexadecimal da cor da luz do jogador.
 *  @default #ffffff
 *  
 *  @param Wall Terrain
 ...
 *  
 *  @param Ceiling Region
 *  @desc ID da regi?o usada em tiles com teto no mapa.
 *  @default 0
 *  
 *  @param 3D Anaglyph
 *  @desc Quando usar ou n?o um efeito de vermelho e ciano para dar profundidade ao jogo. 1 = SIM, 0 = N?O.
 *  @default 0
 *  
 *  @param Texture Filter
 ...
 *  
 *  @param Texture Anisotropy
 *  @desc N?mero de amostras tiradas das texturas. (geralmente uma pot?ncia de 2)
 *  @default 1
 *  
 *  @param Antialiasing
 *  @desc Quando usar antialiasing ou n?o (alto custo de performance). 1 = SIM, 0 = N?O.
 *  @default 0
 *  
 *  @param Optimization Level
 *  @desc N?vel de otimiza??o da cena aplicado (degradante). (none/low/medium/high)
 *  @default none
 *  
 *  @help 
 *  #
 ...
 *                                  por Masked
 *
 *  Esse plugin constr?i um cen?rio 3D para o seu jogo a partir de mapas simples,
 ...
 *  Caso voc? queira criar seu projeto do zero, no entanto, ? necess?rio baixar
 *  o babylon.js de http://cdn.babylonjs.com/2-3/babylon.js e salv?-lo na pasta
 *  "js/libs" do seu jogo. Esse plugin foi feito e testado usando a vers?o r74,
 *  por isso ? recomendado usar esta vers?o para evitar problemas.
 *  Depois, edite o index.html do seu jogo e adicione isso ap?s a linha 14:
 *  <script type="text/javascript" src="js/libs/babylon.js"></script>
 *  Pronto, plugin instalado, agora estamos meio passo mais pr?ximos do final :D
 *
 *  2. Texturas
 *
 *  Texturas s?o definidas criando arquivos com nomes seguindo o padr?o 
 *  "{tileset}_{linha}-{coluna}", substituindo '{tileset}' pelo ID do tileset do
 *  FPLE no database e '{linha}' e '{coluna}' com, respectivamente, o n?mero da 
 *  linha e coluna do tile correspondente ? textura no tileset. Note que apenas
 *  tiles A5 podem ser usados no momento.
 *  Arquivos de textura devem estar na pasta "img/textures" do seu projeto.
 *  N?o h? valor m?nimo ou m?ximo para o tamanho das texturas, mas recomenda-se
 *  usar texturas quadradas e com tamanhos que sejam pot?ncias de 2. ? importante
 *  perceber que usar texturas maiores pode causar lag, especialmente em 
 *  computadores antigos.
 *
 *  3. Tilesets
 * 
 *  Configurar tilesets no FPLE ? bem simples: primeiro, crie um tileset com
 *  apenas a camada A5. Ent?o, configure a passabilidade normalmente e marque tiles
 *  de paede com o terreno que voc? escolheu nas configura??es do plugin.
 *  Note que o primeiro tile da primeira coluna ? sempre invis?vel, ent?o ? 
 *  recomendado deixar isso sinalizado no pr?prio tileset.
 *  
 *  4. Mapeamento
 *  
 *  O mapeamento n?o muda muito, na verdade, a ?nica altera??o ? que tiles sem
 *  textura, vazios ou o primeiro tile da primeira linha do tileset s?o 
 *  transparentes e ficam parecendo buracos no jogo, e voc? pode configurar como
 *  o teto aparece mudando a regi?o dos tiles. Se a regi?o de teto for 0, ent?o
 *  todos os tiles sem regi?o ter?o um teto.
 *  N?o se esque?a de ativar o FPLE no mapa colocando "<fple>" (sem aspas) nas notas dele.
 *
 *  5. Comandos de pugin
 * 
 *  FPLE setResolution <x | v[n]>    : Define a resolu??o do canvas do FPLE
 *                                     (quanto maior, melhor)
 *  FPLE setViewDistance <x | v[n]>  : Define o raio de vis?o do jogador
 *
 *  Exemplos:
 *  FPLE setResolution 0.5           : Define a resolu??o como metade do normal
 *  FPLE setResolution v[42]         : Define a resolu??o como o valor da
 *                                     vari?vel n?mero 42 
 *
 *  FPLE setViewDistance 3           : Define o raio de vis?o como 3 tiles
 *  FPLE setViewDistance v[42]       : Define o raio de vis?o como o valor da
 *                                     vari?vel n?mero 42
 *
 *
 *                                   Cr?ditos
...
(function() {
    ...
    MBS.FPLE.Map.prototype._createCube = function(name, scene) {
        ...
        if (!this._cubeMesh) {
            cube = BABYLON.Mesh.CreateBox(name, 1, scene);
...
MBS.FPLE.Map.prototype._applyTiles1 = function(scene) {
...
(function() {
    ...
    MBS.FPLE.Camera.prototype._updatePosition = function() {
        this.position = new BABYLON.Vector3(-$gamePlayer._realX, 0.8, $gamePlayer._realY);
    };
...
if(this._isFullScreen()){
        ...
        this._fple_canvas.style.width = "81.75%"
}
